...
function App() {
  ...
  const setRegion = useCallback(({ district_id, subdistrict_id, neighbor_id }: setRegionArgs) => {
    ...
    // if (subdistrict_type === 'undefined' && neighbor_type === 'undefined') {
      Connection.get(`/spatilas/district_areas/${district_id}${subdistrict_type !== 'undefined' && `/subdistrict_areas/${subdistrict_id}`}${neighbor_type !== 'undefined' && `/neighbor_areas/${neighbor_id}`}`).then((resp) => {
        setFocusedRegion(resp.data.data);
      })
    // }
  }, []);
  ...
  return (
    <MapInstanceProvider>
      <ModalSnapshotProvider>
        <FocusedRegion.Provider value={{ ...focusedRegion, setRegion }}>
        <Suspense fallback={<></>}>
          <ControlBox />
          <Snapshots />
          <SelectedRegionModal />
